...
// MCP connection URL panel. Replaces the prior "tokens" UI now that the URL
// is no longer a credential. Authentication moved to OAuth — the URL just
// selects which user's permission set applies after the OAuth handshake.

export function UserMcpUrl({
  userId,
  initialMcpUid,
}: {
  userId: string;
  ...
  const url = mcpUid ? `${origin()}/api/mcp/u/${mcpUid}` : null;

  async function onGenerateOrRotate() {
    const verb = mcpUid ? "Rotate" : "Generate";
    if (mcpUid && !confirm("Rotate URL? The old URL stops working immediately — the user will need to re-add the new URL in Claude.")) return;
    setPending(true);
    try {
      const res = await fetch(`/api/users/${userId}/tokens`, { method: "POST" });
      ...
      {url && (
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between text-xs text-muted-foreground">
            <span>
              The URL is not a secret. On first connect, the user signs in via OAuth with their AI
