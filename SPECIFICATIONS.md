...
3. **Pre-call enforcement at `tools/call`**:
   - **For execute-level tools** (signal: tool resolves to `execute` level via admin override or `classifyToolByName()`'s `Execute|Run|Call|Invoke|Exec|Eval` heuristic, plus the legacy `SQL_LIKE_TOOLS` backstop set), the SQL string is extracted from the arguments (recognised keys: `sql`, `query`, `statement`, `soql`, `command`). The SQL is then table-extracted via `extractTablesFromSql()`:
