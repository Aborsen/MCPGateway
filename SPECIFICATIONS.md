...
2. **Pre-call filter at `tools/list`**: if either `allowedTables` or `blockedTables` is in effect for a data source, the listing excludes every **table-bypass tool** — any tool whose resolved permission level is `execute`, plus a small backstop set of known raw-SQL names (`query`, `run_soql`, `execute_ddl`, `run_apex`). The signal is the level, not the name: Skyvia's `Execute`, vendor `RunReport`, custom `eval_*`, etc. all classify as `execute` via `classifyToolByName()` and are caught uniformly. Raw queries cannot be safely bound to a table allowlist.
3. **Pre-call enforcement at `tools/call`**: first, the same table-bypass check from (2) — any execute-level tool is refused outright when restrictions are in effect, with an error like *"Tool 'Execute' (execute) can target any table — refused because this workspace restricts tables to: ..."*. Otherwise, the tool arguments are scanned for a table key — known keys include `table_name`, `table`, `module`, `object_name`, `objectName`, `resource`. If the extracted value is **not** in `allowedTables` (or **is** in `blockedTables`), the call is rejected with a `PERMISSION_DENIED`-style JSON-RPC error before any upstream traffic.
